...
module.exports.on = function on( eventName, pattern, cb ) {
    ...
    if ( !registry[ eventName ] ) {
        ...
        log.verbose( '📣  Now listening event : %s', eventName );
        ...
        emitter.on( eventName, ( message, rinfo ) => {
            ...
            if ( !rcb )
                return log.silly( '🗑  Event dropped : %s %o', eventName, message );

            log.verbose( '📥  Event from %s : %s', rinfo.address, eventName );
    ...
    log.silly( 'Pattern filter added for %s %s', eventName, pattern );
...
module.exports.emit = function emit( eventName, message ) {
    ...
    log.verbose( '📤  Event emited : %s', eventName );
